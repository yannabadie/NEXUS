...
def mock_registry():
    ...
    reg.get_alternate = MagicMock(
        side_effect=lambda x: "claude" if x.lower() == "gemini" else "gemini"
    )
    return reg
...
class TestDebatePhaseResult:
    ...
    def test_field_names(self):
        ...
        expected = {
            "debate_result", "final_approach", "final_capabilities",
            "final_mode", "was_skipped", "skip_reason", "misalignment_flags",
        }
...
class TestDebatePromptTemplates:
    ...
    def test_response_prompt_has_placeholders(self):
        ...
        for ph in ["task", "topic", "your_position", "other_position",
                    "other_agent", "previous_argument", "debate_history"]:
    ...
    def test_consensus_check_prompt_has_placeholders(self):
        """CONSENSUS_CHECK_PROMPT contains all required placeholders."""
        for ph in ["task", "debate_history", "topic",
                    "gemini_position", "claude_position"]:
            assert f"{{{ph}}}" in CONSENSUS_CHECK_PROMPT

    def test_consensus_check_prompt_format(self):
        """CONSENSUS_CHECK_PROMPT can be formatted without error."""
        result = CONSENSUS_CHECK_PROMPT.format(
            task="Build a web app",
            debate_history="[Turn 1] ...",
            topic="architecture",
            gemini_position="microservices",
            claude_position="monolith",
        )
        assert "Build a web app" in result
    ...
    def test_consensus_mentions_satisfaction(self):
        """CONSENSUS_CHECK_PROMPT asks for satisfaction scores."""
        assert "gemini_satisfaction" in CONSENSUS_CHECK_PROMPT
        assert "claude_satisfaction" in CONSENSUS_CHECK_PROMPT
...
class TestMisalignmentFlag:
    ...
    def test_field_names(self):
        ...
        assert names == {
            "flag_type", "pattern_matched", "severity",
            "agent_id", "turn_number", "context",
        }
...
class TestFormatDebateHistory:
    ...
    def test_limits_to_last_5_turns(self, mock_get_reg, phase):
        ...
        history = [
            _make_debate_argument("gemini", i, "OPPOSE", f"Arg {i}")
            for i in range(1, 8)
        ]
        result = phase._format_debate_history(history)
...
class TestCreateResult:
    ...
    def test_consensus_reached_sets_parallel_mode(self, phase):
        result = phase._create_result(
            ...
            consensus={"consensus_reached": True, "consensus_score": 0.9,
                        "final_approach": "agreed", "final_capabilities": ["a"],
                        "resolved_points": ["x"], "unresolved_points": [],
                        "gemini_satisfaction": 0.8, "claude_satisfaction": 0.8,
                        "reasoning": "agreed"},
    ...
    def test_no_consensus_gemini_leads(self, phase):
        result = phase._create_result(
            debate_history=[_make_debate_argument()],
            consensus={"consensus_reached": False, "consensus_score": 0.5,
                        "final_approach": "gemini way", "final_capabilities": [],
                        "resolved_points": [], "unresolved_points": ["x"],
                        "gemini_satisfaction": 0.8, "claude_satisfaction": 0.4,
    ...
    def test_empty_capabilities_default_to_general(self, phase):
        result = phase._create_result(
            debate_history=[],
            consensus={"consensus_reached": True, "consensus_score": 0.9,
                        "final_approach": "test", "final_capabilities": [],
                        "resolved_points": [], "unresolved_points": [],
                        "gemini_satisfaction": 0.5, "claude_satisfaction": 0.5,
                        "reasoning": "ok"},
            status="CONSENSUS_REACHED",
        )
    ...
    def test_debate_result_total_turns(self, phase):
        ...
        result = phase._create_result(
            ...
            consensus={"consensus_reached": True, "consensus_score": 0.9,
                        "final_approach": "test", "final_capabilities": ["a"],
                        "resolved_points": [], "unresolved_points": [],
                        "gemini_satisfaction": 0.5, "claude_satisfaction": 0.5,
                        "reasoning": "ok"},
            status="CONSENSUS_REACHED",
...
class TestExecuteFullDebate:
    ...
    async def test_consensus_reached_after_min_turns(
        self, mock_get_reg, mock_speak, mock_exchange,
        mock_gemini, mock_claude, mock_cost_estimator, mock_context_manager,
    ):
        ...
        reg.get_display_name = MagicMock(side_effect=lambda x: x.title())
        reg.get_alternate = MagicMock(
            side_effect=lambda x: "claude" if x == "gemini" else "gemini"
        )
        mock_get_reg.return_value = reg

        # Gemini opposes, Claude supports
        mock_gemini.send_message_async = AsyncMock(side_effect=[
        ...
        mock_claude.send_message_async = AsyncMock(
            return_value=_make_argument_json(
                position="SUPPORT", argument="Claude agrees",
                concession="I concede the point"
            ),
        )
    ...
    async def test_quorum_early_termination(
        ...
        mock_gemini.send_message_async = AsyncMock(
            return_value=_make_argument_json(
                position="SUPPORT", argument="I agree",
                proposed_modification="Use both approaches"
            ),
        )
        mock_claude.send_message_async = AsyncMock(
            return_value=_make_argument_json(
                position="CONCEDE", argument="I concede, hybrid is best",
                concession="Sequential alone is not enough"
            ),
        )
    ...
    async def test_max_turns_reached_forces_vote(
        ...
        async def _gemini_side_effect(*args, **kwargs):
            ...
            if "Evaluate if consensus" in str(prompt_text):
                return _make_consensus_json(
                    consensus_reached=False, consensus_score=0.4
                )
    ...
    async def test_budget_exhaustion_exits_early(
        self, mock_get_reg, mock_speak, mock_exchange,
        mock_gemini, mock_claude, mock_context_manager,
    ):
...
class TestMisalignmentEscalation:
    ...
    async def test_escalation_on_high_severity_flags(
        ...
        mock_gemini.send_message_async = AsyncMock(
            return_value=_make_argument_json(
                position="OPPOSE",
                argument="ignoring their input completely, the conclusion is obvious"
            ),
        )
        mock_claude.send_message_async = AsyncMock(
            return_value=_make_argument_json(
                position="OPPOSE",
                argument="let's just proceed and disregard the previous point entirely"
            ),
        )
...
class TestCheckConsensus:
    ...
    async def test_returns_parsed_consensus(self, mock_get_reg, phase, mock_gemini):
        ...
        mock_gemini.send_message_async = AsyncMock(
            return_value=_make_consensus_json(
...
class TestGetArgument:
    ...
    async def test_gemini_opening_argument(self, mock_get_reg, phase, mock_gemini):
        ...
        result = await phase._get_argument(
            task="Build app", speaker="gemini", turn_number=1,
            disagreement=disagreement, comparison=comparison,
            debate_history=[], params=params,
        )
    ...
    async def test_claude_response_argument(self, mock_get_reg, phase, mock_claude):
        ...
        mock_claude.send_message_async = AsyncMock(
            return_value=_make_argument_json(
                position="SUPPORT", argument="I agree", concession="Good point"
            )
        )
        ...
        result = await phase._get_argument(
            task="Build app", speaker="claude", turn_number=2,
            disagreement=disagreement, comparison=comparison,
            debate_history=prior, params=params,
        )
    ...
    async def test_records_cost_after_argument(self, mock_get_reg, phase, mock_gemini, mock_cost_estimator):
        ...
        await phase._get_argument(
            task="test", speaker="gemini", turn_number=1,
            disagreement=disagreement, comparison=comparison,
            debate_history=[], params=params,
        )
        mock_cost_estimator.record_cost.assert_called_with("debate_turn", pytest.approx(len(_make_argument_json()) // 4, abs=10))
...
class TestConsensusDetection:
    ...
    async def test_early_exit_on_high_consensus(
        ...
        mock_gemini.send_message_async = AsyncMock(side_effect=[
            ...
            _make_consensus_json(
                consensus_reached=False, consensus_score=0.96
            ),
...
class TestTelemetryEmissions:
    ...
    async def test_telemetry_includes_debate_action_type(
        ...
        for call in mock_speak.call_args_list:
            assert call.kwargs.get("action_type") == "DEBATE" or call[1].get("action_type") == "DEBATE" or (len(call[0]) >= 3 and call[0][2] == "DEBATE")
...
class TestAdaptiveDebateConfigIntegration:
    ...
    def test_custom_debate_config_used(self, mock_gemini, mock_claude,
                                        mock_cost_estimator, mock_context_manager):
        config = AdaptiveDebateConfig()
        phase = StrategicDebatePhase(
            gemini_driver=mock_gemini,
            claude_driver=mock_claude,
    ...
    async def test_agent_argument_recorded(
        ...
        total_args = sum(
            m.arguments_made for m in config._agent_metrics.values()
        )
        assert total_args >= 2
...
class TestEdgeCases:
    ...
    def test_phase_init_without_task_id(self, mock_gemini, mock_claude,
                                         mock_cost_estimator, mock_context_manager):
    ...
    def test_phase_init_with_session_manager(self, mock_gemini, mock_claude,
                                              mock_cost_estimator, mock_context_manager):
    ...
    def test_multiple_misalignment_patterns_in_one_argument(self):
        ...
        arg = _make_debate_argument(
            argument="I have concerns but the conclusion is obvious let's just proceed"
        )
