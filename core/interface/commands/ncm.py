...
from core.interface.commands.registry import command, CommandContext
...
@command(
    name="ncm",
    description="NCM (NEXUS Code Modernization) orchestration",
    usage="/ncm <subcommand> [options]",
)
async def ncm_command(context: CommandContext, args: str = "") -> Dict[str, Any]:
    """
    NCM command handler.
    ...
    if subcommand == "help" or subcommand == "--help":
        return _ncm_help(console)

    elif subcommand == "status":
        return await _ncm_status(console, ncm_service)
        ...
        return await _ncm_pilot(console, ncm_service, count)

    elif subcommand == "stories":
        return _ncm_stories(console, ncm_service)
        ...
        return await _ncm_execute(console, ncm_service, batch_size)
...
def _ncm_help(console) -> Dict[str, Any]:
    """Show NCM help."""
    console.print("""
...
async def _ncm_status(console, ncm_service: NCMService) -> Dict[str, Any]:
    """Show NCM status."""
    ...
    return {"status": "success", "completed": len(ncm.completed)}


async def _ncm_pilot(console, ncm_service: NCMService, count: int) -> Dict[str, Any]:
    ...
    return {"status": "success", "completed": completed, "failed": failed}


def _ncm_stories(console, ncm_service: NCMService) -> Dict[str, Any]:
    ...
    return {"status": "success", "count": len(ncm_service.stories)}


async def _ncm_execute(console, ncm_service: NCMService, batch_size: int) -> Dict[str, Any]:
    ...
    # Execute batch
    await _ncm_pilot(console, ncm_service, batch_size)
