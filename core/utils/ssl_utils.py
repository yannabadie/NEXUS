"""SSL helpers for enterprise CA bundles and safe fallbacks."""
...
class SslConfig:
    """SSL configuration with optional enterprise CA bundle."""
    ...
    def from_env(cls) -> "SslConfig":
        ssl_mode = os.getenv("NEXUS_SSL_MODE", "strict").lower()
        ca_bundle = (
        ...
        if not ca_bundle_path:
            ca_bundle_path = auto_windows_ca_bundle()
        return cls(ssl_mode=ssl_mode, ca_bundle_path=ca_bundle_path)


def build_ssl_context(config: SslConfig) -> ssl.SSLContext:
    """Build an SSL context with optional CA bundle overrides."""
...
def urlopen_ssl(
    ...
    """Open URL with SSL handling and optional enterprise fallback."""
    config = ssl_config or SslConfig.from_env()
    ...
    try:
        ...
        if config.ssl_mode in {"auto", "enterprise"}:
            fallback = SslConfig(ssl_mode="insecure", ca_bundle_path=None)
            return urllib.request.urlopen(
                request_or_url,
                timeout=timeout,
                context=build_ssl_context(fallback),
            )
        raise


def auto_windows_ca_bundle() -> Optional[Path]:
    """Export Windows root/CA certificates into a PEM bundle."""
    if sys.platform != "win32":
        return None
    workspace = Path(os.getenv("WORKSPACE_PATH", "./workspace"))
    bundle_path = workspace / "ssl" / "corp_ca_bundle.pem"
    refresh = os.getenv("NEXUS_CA_REFRESH", "false").lower() == "true"
