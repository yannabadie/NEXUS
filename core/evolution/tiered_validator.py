...
"""
    ...
    def _run_benchmark_dimension(self, dimension: str) -> Dict:
        """
        Run a single benchmark dimension (V7: real benchmarks).

        Uses ASIBenchmark for dynamic runtime testing:
        - coding: Real Python task execution
        - reasoning: Logic puzzle validation (V7)
        - creativity: Novel solution validation (V7)
        - scalability: Static heuristic (MVP)
        """
        try:
            # Import ASI benchmark from project root
            benchmark_path = self.child_path.parent.parent / "BENCHMARKS"
            if str(benchmark_path.parent) not in sys.path:
                sys.path.insert(0, str(benchmark_path.parent))

            from BENCHMARKS.asi_benchmark import ASIBenchmark

            benchmark = ASIBenchmark(self.child_path)

            # Route to appropriate benchmark method
            if dimension == "coding":
                result = benchmark._run_coding_benchmark(parallel=True)
            elif dimension == "reasoning":
                result = benchmark._run_reasoning_dynamic()
            elif dimension == "creativity":
                result = benchmark._run_creativity_dynamic()
            else:  # scalability - keep heuristic for MVP
                result = benchmark._run_scalability_heuristic()

            return {
                "score": result.score,
                "method": "runtime" if dimension in ["coding", "reasoning", "creativity"] else "heuristic",
            ...
            import random
            print(f"[TIERED_VALIDATOR] Benchmark {dimension} failed: {e}, using heuristic")
            return {"score": 0.5 + random.uniform(-0.1, 0.1), "method": "heuristic_fallback", "error": str(e)}
