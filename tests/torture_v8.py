...
class TortureProtocolV8(TortureBase):
    ...
    def generate_report(self):
        ...
        print(f"Hot-Swap Effect:  {rates['hot_swap_effectiveness']:.1f}% (target: >{self.TARGETS['hot_swap_effectiveness']}%)")
...
def main():
    ...
    parser.add_argument(
        "--include-slow",
        action="store_true",
        help="Include slow/stress tests"
    )
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output path for JSONL metrics"
    )
    parser.add_argument(
        "--category",
        type=str,
    ...
    try:
        from tests.torture.chaos_injectors import CrashInjector
        print("[OK] torture.chaos_injectors")
    except ImportError as e:
        print(f"[FAIL] torture.chaos_injectors: {e}")

    try:
        from tests.torture.scenarios.saga_crash import test_cr001_partial_checkpoint_write
        print("[OK] torture.scenarios.saga_crash (15 tests)")
    except ImportError as e:
        print(f"[FAIL] torture.scenarios.saga_crash: {e}")

    try:
        from tests.torture.scenarios.saga_concurrency import test_cc001_simultaneous_checkpoints
        print("[OK] torture.scenarios.saga_concurrency (12 tests)")
    except ImportError as e:
        print(f"[FAIL] torture.scenarios.saga_concurrency: {e}")

    try:
        from tests.torture.scenarios.context_edge import test_ce001_context_index_out_of_bounds
        print("[OK] torture.scenarios.context_edge (10 tests)")
    except ImportError as e:
        print(f"[FAIL] torture.scenarios.context_edge: {e}")

    try:
        from tests.torture.scenarios.compensation import test_cf001_compensation_exception
        print("[OK] torture.scenarios.compensation (8 tests)")
    except ImportError as e:
        print(f"[FAIL] torture.scenarios.compensation: {e}")

    try:
        from tests.torture.scenarios.hive_integration import test_hm001_crash_between_checkpoint_and_context
