...
Supports:
    ...
    - Actions: 'overwrite', 'append', 'replace'
...
[
  {
    ...
    "action": "overwrite",  // "append", "replace"
    "target": "string to replace", // required for 'replace'
    "change": "new content",
...
def load_mutations(json_path: Path) -> List[Dict[str, Any]]:
    ...
    for i, mutation in enumerate(mutations):
        ...
        if mutation["action"] not in ("overwrite", "append", "replace"):
            raise ValueError(f"Mutation {i+1}: Invalid action '{mutation['action']}'. Must be 'overwrite', 'append', or 'replace'")
            
        # Validate replace requirements
        if mutation["action"] == "replace" and "target" not in mutation:
            raise ValueError(f"Mutation {i+1}: 'replace' action requires 'target' field")
...
def apply_mutation_to_target(mutation: Dict[str, Any], target_dir: Path, index: int) -> None:
    ...
    Actions:
        ...
        - 'replace': Replace 'target' string with 'change' string
    """
    ...
    # Handle file creation if it doesn't exist (only for overwrite/append)
    if not target_file.exists():
        if action == "append":
            print(f"[WARN] Mutation {index}: Target file {target_file} does not exist. Creating empty file for append.")
            target_file.touch()
        elif action == "replace":
             raise FileNotFoundError(f"Mutation {index}: Cannot replace in non-existent file {target_file}")
    ...
    if action == "overwrite":
        ...
        if target not in original_content:
            # Try strict normalization (ignoring trailing whitespace on lines)
            # But for now, exact match is safest.
            print(f"[ERROR] Mutation {index}: Target string not found in {target_file}")
            print(f"Target (first 100 chars): {target[:100]}")
            raise ValueError(f"Mutation {index}: Target string not found for replacement")
            
        new_content = original_content.replace(target, change)
        print(f"[INFO] Mutation {index}: REPLACE in {target_file}")

    # Validate Python syntax BEFORE writing
    print(f"Applying mutation to {target_file}: {mutation['action']}")
...
def main():
    if len(sys.argv) != 3:
        ...
        print("\nMutation JSON Schema:")
        print('[{"file": "path", "action": "overwrite|append|replace", "target": "old", "change": "new", "reason": "why"}]')
        sys.exit(1)
