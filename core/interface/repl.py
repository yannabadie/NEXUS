...
""")
    ...
    def brainstorm_children_with_ais(self, parent_id: str, parent_path: Path, child_count: int) -> list:
        ...
        # V7.1: Load prompt from file (separation of concerns)
        prompt_path = self.nexus_root / "prompts" / "evolution_brainstorm.md"
        if not prompt_path.exists():
             self.console.print_error(f"Missing prompt file: {prompt_path}")
             return []
             
        brainstorm_template = prompt_path.read_text(encoding="utf-8")
        
        # Craft the EMERGENT brainstorming task
        brainstorm_task = brainstorm_template.format(
...
PRODUISEZ LES BLOCS DE MUTATION MAINTENANT ({child_count} mutations requises).""",
    ...
    def brainstorm_spinoff_with_ais(self, parent_id: str, parent_path: Path, mission: str) -> list:
        ...
        # V7.1: Load prompt from file (separation of concerns)
        prompt_path = self.nexus_root / "prompts" / "specialization_mission.md"
        if not prompt_path.exists():
             self.console.print_error(f"Missing prompt file: {prompt_path}")
             return []

        specialization_template = prompt_path.read_text(encoding="utf-8")
        
        # Craft the SPECIALIZATION task
        brainstorm_task = specialization_template.format(
            mission=mission
        )
